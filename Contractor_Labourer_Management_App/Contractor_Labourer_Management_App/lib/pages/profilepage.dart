...
import 'package:flutter_fire/services/usermanagement.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future uploadImage() async {
    ...
    final StorageReference firebaseStorageRef = FirebaseStorage.instance
        .ref()
        .child('profilepics/${randomno.nextInt(500000).toString()}.jpg');
    StorageUploadTask uploadTask = firebaseStorageRef.putFile(selectedImage);
    await uploadTask.onComplete;
    print('File Uploaded.!');
    firebaseStorageRef.getDownloadURL().then((picUrl) {
      setState(() {
        userManagement.updateProfilePic(picUrl.toString()).then((val) {
          setState(() {
            profilePicUrl = picUrl.toString();
            isLoading = false;
          });
        });
      });
  ...
  Future<bool> editName(BuildContext context) async {
    return showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Enter New Name', style: TextStyle(fontSize: 15.0)),
            content: Container(
              height: 100.0,
              width: 100.0,
              child: Column(
